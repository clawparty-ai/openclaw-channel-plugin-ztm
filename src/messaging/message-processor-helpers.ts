...
export { getWatermarkKey, type WatermarkKeyInput } from './watermark.js';
...
export async function processAndNotifyPeerMessages(
  ...
  for (const msg of messages) {
    ...
    // Check DM policy before processing
...
export async function processAndNotifyGroupMessages(
  ...
  for (const msg of messages) {
    ...
    // Check group policy before processing
    const policyResult = checkMessagePolicy({
      sender: msg.sender,
      content: msg.message,
      config: state.config,
    ...
    if (normalized) {
      ...
      await notifyMessageCallbacks(state, messageWithGroup);
    }
  }
}
