...
describe("normalizeMessageContent", () => {
  ...
  describe("nested message format", () => {
    it("should extract text from {message: {text: string}}", () => {
      expect(
        normalizeMessageContent({ message: { text: "Nested message" } })
      ).toBe("Nested message");
    });
    ...
    it("should return JSON for nested non-string text", () => {
      expect(
        normalizeMessageContent({ message: { text: 123 } })
      ).toBe('{"text":123}');
    });
    ...
    it("should return JSON for nested null message", () => {
      ...
      expect(
        normalizeMessageContent({ message: null })
      ).toBe('{"message":null}');
    });
  });
  ...
  describe("complex objects", () => {
    it("should stringify object with multiple properties", () => {
      expect(
        normalizeMessageContent({ foo: "bar", baz: 123 })
      ).toBe('{"foo":"bar","baz":123}');
    });
