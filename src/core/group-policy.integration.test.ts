...
import { checkGroupPolicy, getGroupPermission, applyGroupToolsPolicy } from './group-policy.js';
...
describe('Group Policy Integration', () => {
  describe('full permission check flow', () => {
    ...
    it('should handle full flow: open group with mention required', () => {
      ...
      const perm1 = getGroupPermission(msgNoMention.groupCreator!, msgNoMention.groupId!, config);
      const result1 = checkGroupPolicy(msgNoMention.sender, msgNoMention.content, perm1, 'chatbot');
  ...
  describe('tools policy integration', () => {
    ...
    it('should use default tools when no config', () => {
      ...
      const allTools = ['group:messaging', 'group:sessions', 'group:runtime', 'group:fs', 'exec'];
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters in group id', () => {
      ...
      const perm = getGroupPermission('alice', 'group+with-special@chars', config);
    ...
    it('should handle case variations in mention', () => {
      ...
      // 各种大小写变体（需要 @ 符号）
      expect(checkGroupPolicy('bob', '@CHATBOT hello', perm, 'chatbot').allowed).toBe(true);
      expect(checkGroupPolicy('bob', '@ChatBot hello', perm, 'chatbot').allowed).toBe(true);
      expect(checkGroupPolicy('bob', '@chatbot hello', perm, 'chatbot').allowed).toBe(true);
    });
    ...
    it('should handle non-mention text that looks like username', () => {
      ...
      expect(checkGroupPolicy('bob', 'hey chatbot how are you?', perm, 'chatbot').allowed).toBe(
        false
      );
    });
  });
