...
const GroupPermissionsSchema = Type.Object({
  ...
  groupPolicy: Type.Optional(
    Type.Union([Type.Literal('open'), Type.Literal('disabled'), Type.Literal('allowlist')], {
      default: 'allowlist',
    })
  ),
...
export const ZTMChatConfigSchema = Type.Object(
  {
    ...
    allowFrom: Type.Optional(
      Type.Array(
        ...
        }),
        {
          ...
          description:
            'Whitelist of usernames allowed to send messages (empty = allow all paired users)',
        }
      )
    ),
    ...
    groupPermissions: Type.Optional(
      Type.Record(
